...
```
# Поиск готового образа simple-http-server
```bash
docker run -d \
  --name file-server \
  -p 8084:80 \
...
```
##  Получение готового образа simple-http-server

Получить информацию по загруженному образу:
```bash
docker inspect file-server
```
...
```bash
docker restart file-server
```
Перезапустить контейнер по его id
```bash
docker restart fca65c1652cf
```
...
И можно удалить ещё и образ загруженного ранее simple-http-server:
...
```bash
docker rmi fca65c1652cf
```
![alt text](image-37.png)

## Проверить работу контейнера

Можно снова установить и запустить simple-http-server (если его удаляли ранее)
```bash
docker run -d \
  --name file-server \
  -p 8084:80 \
...
Зайти в контейнер
```bash
docker exec -it file-server /bin/bash
```
или
```bash
