...
```
# Поиск готового образа Pcb2gcode
```bash
...
```bash
docker rm hardcore_shtern
```
![alt text](image-36.png)

И можно удалить ещё и образ загруженного ранее Pcb2gcode:
...
Можно снова установить и запустить Pcb2gcode (если его удаляли ранее)
```bash
docker run --rm -p 8081:5000 -d \
  -e URL=http://localhost \
  -e RPORT=8180 \
...
Проверить порт 8081 для Linux/Mac/WSL:
```bash
...
```bash
docker logs hardcore_shtern
```
Показать логи в режиме ожидания
```bash
docker logs -f hardcore_shtern
```
...
Зайти в контейнер
```bash
docker exec -it hardcore_shtern /bin/bash
```
или
```bash
docker exec -it hardcore_shtern bash
```
или
```bash
docker exec -it hardcore_shtern /bin/sh
```
или
```bash
docker exec -it hardcore_shtern sh
```
